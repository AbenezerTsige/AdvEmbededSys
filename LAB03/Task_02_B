...
int main(void)
{
    ...
    // Before calling any peripheral specific driverLib function, we must enable the clock
    ...
    while(1)
    {
        ...
        // R = 2, G = 8, B = 4
        // RG = 10 (yellow), RB = 6 (purple) , GB = 12 (cyan) , RGB = 14 (white) , R, G, B

        if(ui8PinData == 8 ) {
            ui8PinData = ui8PinData / 2 ;}
        else {
            ui8PinData = ui8PinData + 6;
            if(ui8PinData == 16 ) {ui8PinData = 6;}
            if(ui8PinData == 18 ) {ui8PinData = 14;}
            if(ui8PinData == 20 ) {ui8PinData = 2;}
            }
